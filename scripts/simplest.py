...
A, As, B, C, Cs, D, Ds, E, F, Fs, G, Gs = [
    55.00 * math.pow(2, (i / 12)) for i in range(12)
]  # Hz
A *= 2
As *= 2
B *= 2
...
def play(
    ...
    if shape == "q":  # square wave
        for i in range(frames):      
            value = 2000 if int(float(i) / sampleRate * 2 * note) % 2 == 0 else -2000
            data = struct.pack("<h", value)
            file.writeframesraw(data)
    elif shape == "s":  # sine wave
        for i in range(frames):
            value = int(6000 * math.sin(2 * math.pi * note * float(i) / sampleRate))
            data = struct.pack("<h", value)
            file.writeframesraw(data)
